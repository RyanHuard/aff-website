...
def query_season_stat_leaders(season_id):
    ...
    SELECT 
        ...
        ps.position, 
        ps.number, 
        ROUND(AVG(ps.match_pass_yards), 1) AS season_pass_yards,
        t.team_logo
    FROM 
        player_stats ps
    JOIN 
        teams t ON ps.team_city = t.abbreviation
    WHERE 
    ...
    GROUP BY 
        ...
        t.team_logo
    ORDER BY 
        season_pass_yards DESC
    LIMIT 5;
    """

    # Pass Touchdowns Query
    pass_tds_query = f"""
    SELECT 
        ps.first_name, 
        ps.last_name, 
        ps.position, 
        ps.number, 
        SUM(ps.match_pass_tds) AS season_pass_tds,
        t.team_logo
    FROM 
        player_stats ps
    JOIN 
    ...
    SELECT 
        ...
        ps.position, 
        ps.number, 
        ROUND(AVG(ps.match_rush_yards), 1) AS season_rush_yards,
        t.team_logo
    FROM 
        player_stats ps
    JOIN 
    ...
    GROUP BY 
        ...
        t.team_logo
    ORDER BY 
        season_rush_yards DESC
    LIMIT 5;
    """

    # Receiving Yards Query
    receiving_yards_query = f"""
    SELECT 
        ps.first_name, 
        ps.last_name, 
        ps.position, 
        ps.number, 
        ROUND(AVG(ps.match_receiving_yards), 1) AS season_receiving_yards,
        t.team_logo
    FROM 
        player_stats ps
    JOIN 
    ...
    GROUP BY 
        ...
        t.team_logo
    ORDER BY 
        season_receiving_yards DESC
    LIMIT 5;
    """

    # Total Touchdowns Query
    total_touchdowns_query = f"""
    SELECT 
        ps.first_name, 
        ps.last_name, 
        ps.position, 
        ps.number, 
        (SUM(ps.match_receiving_tds) + SUM(ps.match_rush_tds)) AS season_total_touchdowns,
        t.team_logo
    FROM 
        player_stats ps
    JOIN 
    ...
    GROUP BY 
        ...
        t.team_logo
    ORDER BY 
        season_total_touchdowns DESC
    LIMIT 5;
    """

    # Defense Sacks Query
    defense_sacks_query = f"""
    SELECT 
        ps.first_name, 
        ps.last_name, 
        ps.position, 
        ps.number, 
        SUM(ps.match_defense_sacks) AS season_defense_sacks,
        t.team_logo
    FROM 
        player_stats ps
    JOIN 
